...
exports.loginAdminService = async (data, res) => {
  ...
  if (isPassValid) {
    ...
    if (user.schoolId) {
      ...
      const school = await School.findById(user.schoolId).select("features");
      if (school) {
        schoolFeatures = school.features;
      }
    }

    // Generate a token with schoolId and role for multi-tenancy
    ...
    const result = {
      user: {
        ...
        features: schoolFeatures, // Attach features here
      },
      token,
    };
